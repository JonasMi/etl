...
public class InhospitalDiagnosisDetails extends InhospitalDiagnosisRecord {
    /**
     *名称:院内诊断记录id
     *备注:本次诊断在院内系统中的唯一标识
 	 *
     ...
     */
     ...
     public InhospitalDiagnosisDetails(InhospitalDiagnosis inhospitalDiagnosis) {
        super(inhospitalDiagnosis);
         this.diagnosisId=inhospitalDiagnosis.getDiagnosisId();
         this.patientId=inhospitalDiagnosis.getPatientId();
         this.patientName=inhospitalDiagnosis.getPatientName();
         this.inpatientRegistrationId=inhospitalDiagnosis.getInpatientRegistrationId();
         this.diagnosisEmpId=inhospitalDiagnosis.getDiagnosisEmpId();
         this.diagnosisCd=inhospitalDiagnosis.getDiagnosisCd();
         this.diagnosisValue=inhospitalDiagnosis.getDiagnosisValue();
         java.util.Date _onsetdatetime = inhospitalDiagnosis.getOnsetdatetime();
         if(_onsetdatetime!=null){
         	this.onsetdatetime=_onsetdatetime.getTime();
         }
         this.tcmWesternCd=inhospitalDiagnosis.getTcmWesternCd();
         this.inhosIllnessCd=inhospitalDiagnosis.getInhosIllnessCd();
         this.diagResult=inhospitalDiagnosis.getDiagResult();
         this.propertyCd=inhospitalDiagnosis.getPropertyCd();
         this.diagExplan=inhospitalDiagnosis.getDiagExplan();
         this.admitDiag=inhospitalDiagnosis.getAdmitDiag();
         this.errorMessage=inhospitalDiagnosis.getErrorMessage();
         this.errorDetail=inhospitalDiagnosis.getErrorDetail();      
    }
