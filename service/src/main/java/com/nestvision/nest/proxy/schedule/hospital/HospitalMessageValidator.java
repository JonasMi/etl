package com.nestvision.nest.proxy.schedule.hospital;

import org.springframework.stereotype.Component;
import com.nestvision.nest.proxy.base.util.JSONUtil;
import com.nestvision.nest.proxy.fhir.entity.Hospital;
import com.nestvision.nest.proxy.schedule.MessageValidateResult;
import com.nestvision.nest.proxy.schedule.MessageValidator;
import com.nestvision.nest.proxy.schedule.MessageValidatorSupport;
import lombok.extern.slf4j.Slf4j;

@Component
@Slf4j
public class HospitalMessageValidator extends MessageValidatorSupport implements MessageValidator<Hospital>{
    @Override
    public MessageValidateResult validate(Hospital hospital) {
        MessageValidateResult messageValidateResult;
        try {
            messageValidateResult = doValidate(hospital);
            if(log.isDebugEnabled()){
                String result =JSONUtil.toJSONString(messageValidateResult);             
                log.debug("Hospital[{}]消息校验结果:[{}]",  hospital.getUid(),result);
            }           
            return messageValidateResult;
        } catch (Exception e) {
			 log.error("在校验Hospital消息的时候发生错误:"+e.getMessage(),e);
            throw new RuntimeException(e);
        }
    }

}
