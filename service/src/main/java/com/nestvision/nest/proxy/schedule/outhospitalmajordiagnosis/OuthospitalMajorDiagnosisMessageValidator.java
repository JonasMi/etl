package com.nestvision.nest.proxy.schedule.outhospitalmajordiagnosis;

import org.springframework.stereotype.Component;
import com.nestvision.nest.proxy.base.util.JSONUtil;
import com.nestvision.nest.proxy.fhir.entity.OuthospitalMajorDiagnosis;
import com.nestvision.nest.proxy.schedule.MessageValidateResult;
import com.nestvision.nest.proxy.schedule.MessageValidator;
import com.nestvision.nest.proxy.schedule.MessageValidatorSupport;
import lombok.extern.slf4j.Slf4j;

@Component
@Slf4j
public class OuthospitalMajorDiagnosisMessageValidator extends MessageValidatorSupport implements MessageValidator<OuthospitalMajorDiagnosis>{
    @Override
    public MessageValidateResult validate(OuthospitalMajorDiagnosis outhospitalMajorDiagnosis) {
        MessageValidateResult messageValidateResult;
        try {
            messageValidateResult = doValidate(outhospitalMajorDiagnosis);
            if(log.isDebugEnabled()){
                String result =JSONUtil.toJSONString(messageValidateResult);             
                log.debug("OuthospitalMajorDiagnosis[{}]消息校验结果:[{}]",  outhospitalMajorDiagnosis.getUid(),result);
            }           
            return messageValidateResult;
        } catch (Exception e) {
			 log.error("在校验OuthospitalMajorDiagnosis消息的时候发生错误:"+e.getMessage(),e);
            throw new RuntimeException(e);
        }
    }

}
