			...
			<Composition xmlns="http://hl7.org/fhir">
				...
				<contained>
					<Condition xmlns="http://hl7.org/fhir">
						...
						<#if outhosCnSymDiagCode??>
						<!--中医症候代码CV9900357 -->
						<extension
							url="http://www.nestvision.com/platform/app/data/v1/code/cn-sym-diag-code">
							<valueCode value="${outhosCnSymDiagCode}" />
						</extension>
						</#if>
						<#if outhosCnDisDiagCode??>
						<!--中医病名代码CV9900357 -->
						<extension
							url="http://www.nestvision.com/platform/app/data/v1/code/cn-dis-diag-code">
							<valueCode value="${outhosCnDisDiagCode}" />
						</extension>
						</#if>
						<#if outhosDiagId??>
						<identifier>
				...
				<identifier>
					<system
						value="http://www.nestvision.com/entity/hospital/${upHospital}/${developerSystemType}/${developer}/v1/identifier/admission-discharge-record-id" />
					<value value="${admissionDischargeRecordId}" />
				</identifier>
				<!-- 记录日期 -->
				...
				<type>
					...
					<text value="24h内入出院记录" />
				</type>
				<!-- 文档标题：24h内入出院记录 -->
				<title value="24h内入出院记录" />
				...
				<section>
					...
					<!-- 症状名称 -->
					<extension
						url="http://www.nestvision.com/platform/app/data/v1/text/symptom-name">
						<valueString value="${symptomName}" />
					</extension>
					</#if>
