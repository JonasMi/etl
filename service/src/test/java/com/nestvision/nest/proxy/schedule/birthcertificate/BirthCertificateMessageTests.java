package com.nestvision.nest.proxy.schedule.birthcertificate;

import java.util.UUID;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;
import com.nestvision.nest.proxy.fhir.entity.BirthCertificate;
import com.nestvision.nest.proxy.fhir.service.BirthCertificateService;
import com.nestvision.nest.proxy.test.EntityGenarator;

@RunWith(SpringRunner.class)
@SpringBootTest
public class BirthCertificateMessageTests {

    @Autowired
    private BirthCertificateService birthCertificateService;

    @Autowired
    private BirthCertificateMessageScheduler birthCertificateMessageScheduler;

    @Autowired
    private BirthCertificateMessageSender birthCertificateMessageSender;

    @Test
    public void testTemplateProcess() throws Exception {

        BirthCertificate birthCertificate = EntityGenarator.genarate(BirthCertificate.class);
		birthCertificate.setUid(UUID.randomUUID().toString()); 
		 
        String templateResult = birthCertificateMessageSender.createRequestBody(birthCertificate);
        System.out.println("#####完整消息实体请求模板处理结果显示#####");
        System.out.println(templateResult);
        System.out.println("#####完整消息实体请求模板处理结果显示#####");

        birthCertificate = EntityGenarator.genarateNessaray(BirthCertificate.class);
        templateResult = birthCertificateMessageSender.createRequestBody(birthCertificate);
        System.out.println("#####必要消息实体请求模板处理结果显示#####");
        System.out.println(templateResult);
        System.out.println("#####必要消息实体请求模板处理结果显示#####");

    }

    @Test
    public void testSend() throws Exception{
    	
    	//发送完整消息
		BirthCertificate birthCertificate = EntityGenarator.genarate(BirthCertificate.class);	
		String uid = UUID.randomUUID().toString();
		birthCertificate.setUid(uid);  		 		 
        birthCertificateService.save(birthCertificate);
        
        birthCertificate=birthCertificateService.findOne(uid);      
        birthCertificateMessageScheduler.doSend(birthCertificate);
        System.out.println("#####完整消息实体请求已经发送完毕#####");
  
        //发送必要消息
        birthCertificate = EntityGenarator.genarateNessaray(BirthCertificate.class);
        uid = UUID.randomUUID().toString();
        birthCertificate.setUid(uid);
        birthCertificateService.save(birthCertificate);
        
        birthCertificate=birthCertificateService.findOne(uid);      
        birthCertificateMessageScheduler.doSend(birthCertificate);
        System.out.println("#####必要消息实体请求已经发送完毕#####");
        
    }
}
